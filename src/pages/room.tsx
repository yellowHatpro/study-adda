...
export const Room = () => {
    ...
    return (
        <Layout
            navbarElements={<IDE_Draw_Switch setIDE={setIsIDE} />}>
            <div className={"container flex items-start gap-1"}>
                <aside className={"fixed top-14 z-30 hidden shrink-0 md:sticky md:block"}>
        ...
        <Tabs defaultValue={"ide"} className={"w-[120px]"}>
            <TabsList className={"grid w-full grid-cols-2"}>
                <TabsTrigger
                    ...
                    value={"ide"}>IDE</TabsTrigger>
                <TabsTrigger
                    onClick={()=>{
                        setIDE(false)
                    }}
                    value={"draw"}>Draw</TabsTrigger>
            </TabsList>
        </Tabs>
    )
}
